...
@section('content')
    ...
    <form action="{{ route('admin.category.insert', $category->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="form-group" style="margin-bottom: 10px">
            <label for="name" style="margin-bottom: 0;">Kategori İsmi</label>
            @foreach ($languages as $language)
                <div style="display:flex; align-items: center">
                    ...
                    <div style="flex: 1;">
                        <input value='{{ old('name.' . $language->id, $category['details'][$language->id]->name) }}' name='name[{{ $language->id }}]' type="text"
                            class="form-control" id='name[{{ $language->id }}]'
                            placeholder="Kategori ismi giriniz ({{ $language->name }})"
        ...
        <div class="form-group" style="margin-bottom: 10px">
            ...
            @foreach ($languages as $language)
                <div style="display:flex; align-items: center">
                    ...
                    <div style="flex: 1;">
                        <input value='{{ old('meta_title.' . $language->id, $category['details'][$language->id]->meta_title) }}' name='meta_title[{{ $language->id }}]' type="text"
        ...
        <div class="form-group" style="margin-bottom: 10px">
            ...
            @foreach ($languages as $language)
                <div style="display:flex; align-items: center">
                    ...
                    <div style="flex: 1;">
                        <input value='{{ old('meta_description.' . $language->id, $category['details'][$language->id]->meta_description) }}' name='meta_description[{{ $language->id }}]'
        ...
        <div class="form-group" style="margin-bottom: 10px">
            ...
            @foreach ($languages as $language)
                <div style="display:flex; align-items: center">
                    ...
                    <div style="flex: 1;">
                        <input value='{{ old('meta_keywords.' . $language->id, $category['details'][$language->id]->meta_keywords) }}' name='meta_keywords[{{ $language->id }}]' type="text"
        ...
        <div class="form-group" style="margin-bottom: 10px">
            ...
            @foreach ($languages as $language)
                <div style="display:flex; align-items: center">
                    ...
                    <div style="flex: 1;">
                        <input value='{{ old('slug.' . $language->id, $category['details'][$language->id]->slug) }}' name='slug[{{ $language->id }}]' type="text"
        ...
        <div class="form-group" style="margin-bottom: 10px">
            ...
            <input value='{{ old('sort_order', $category['details'][$language->id]->sort_order) }}' name='sort_order' type="text" class="form-control" id='sort_order'
        ...
        <div class="form-group" style="margin-bottom: 10px">
            ...
            <select name="parent_id" class="select2" style="width: 100%">
                ...
                @foreach ($categories as $item)
                    <option value="{{ $item->id }}" @if ($item->id==$category->parent_id) selected @endif> {{ $item->details->name }}</option>
                @endforeach
            </select>
            <div style="color:red;"> @error('parent_id')
                    {{ $message }}
                @enderror
            </div>
        </div>
        <div class="form-group" style="margin-bottom: 10px">
            ...
            @foreach ($languages as $language)
                <div style="display:flex; align-items: center">
                    ...
                    <div style="flex: 1;">
                        <textarea name='description[{{ $language->id }}]'
                            id='content{{ $language->id }}'>{{ old('description.' . $language->id, $category['details'][$language->id]->description) }}
                        </textarea>
                    </div>
                </div>
